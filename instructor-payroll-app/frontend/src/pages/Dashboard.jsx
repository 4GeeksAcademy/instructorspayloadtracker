...
export default function Dashboard() {
  ...
  const createPeriod = async (payload) => {
    const res = await client.post("/periods", payload);
    // Periods are returned newest-first everywhere else in the app; keep
    // that order here and jump straight to the period that was just made.
    setPeriods((prev) => [res.data, ...prev].sort((a, b) => (a.start_date < b.start_date ? 1 : -1)));
    ...
    <>
      ...
      {periods.length > 0 && (
        <PeriodSelector
          ...
          canCreate={user.role === "owner" || user.role === "admin"}
